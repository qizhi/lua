...
/**
 * Simple test class for running a single master node service with
 * id "mas1" within a pre-setup environment, such as eclipse. A second
 * argument pointing to the config folder is accepted.
 * 
 * @author lars.nilsson
 */
public class MasterServer {
    ...
    public static void main(String[] args) {
    	try {
	    	...
	    	inst.init(checkId(args), checkInitFolder(args), new SharedClassLoader(MasterServer.class.getClassLoader()), new BootstrapNode[] { new BootstrapNode("master", checkId(args)) });
	    	inst.start();
