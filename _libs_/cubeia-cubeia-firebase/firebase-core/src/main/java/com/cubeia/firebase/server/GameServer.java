...
/**
 * Simple test class for running a single game node service with
 * id "gam1" within a pre-setup environment, such as eclipse.
 * 
 * <p>For multiple game nodes, supply another id as a first argument. A 
 * second argument pointing to the config folder is accepted.
 * 
 * @author lars.nilsson
 */
public class GameServer {
    ...
    public static void main(String[] args) {
    	try {
	    	...
	    	inst.init(checkId(args), checkInitFolder(args), new SharedClassLoader(GameServer.class.getClassLoader()), new BootstrapNode[] { new BootstrapNode("game", checkId(args)) });
	    	inst.start();
