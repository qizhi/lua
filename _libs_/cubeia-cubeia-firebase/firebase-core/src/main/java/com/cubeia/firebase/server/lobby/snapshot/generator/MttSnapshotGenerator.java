...
public class MttSnapshotGenerator extends SnapshotGenerator {

	@SuppressWarnings("unused")
	private static transient Logger log = Logger.getLogger(MttSnapshotGenerator.class);
	
	private ConcurrentMap<Integer, LobbyPath> idToPathMap = new ConcurrentHashMap<Integer, LobbyPath>();
	
	public MttSnapshotGenerator(Lobby lobby, LobbyPathType type) {
		super(lobby, type, "mtt_snapshot_generator");
	}
	
	public void nodeAttributeChanged(NodeChangeDTO change) {
		if (change.getPath().getType().equals(LobbyPathType.MTT)) {
