...
public class GameQueue extends PartitionedQueue {

	public GameQueue(PartitionedQueues repo) {
		super(repo);
	}

	@Override
	protected StrictPooledHandoff getObjectHandoff() {
		return repo.getGameHandoff();
	}

	@Override
	protected StrictQueueReceiver<String> getObjectReceiver(int id) {
		return repo.getGameReceivers().get(id);
	}
	
	@Override
	protected OrphanEventListener<ChannelEvent> getOrphanListener() {
		return repo.getGameOrphanListener();
	}
}
