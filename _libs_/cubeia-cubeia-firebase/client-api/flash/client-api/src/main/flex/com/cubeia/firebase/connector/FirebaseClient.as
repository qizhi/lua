...
﻿package com.cubeia.firebase.connector
{
	...
	import com.cubeia.firebase.comm.SendLater;
...
public function onLoginEvent(loginEvent:LoginEvent):void
{
	...
	*/
	public class FirebaseClient extends EventDispatcher implements SocketDataHandler
	{
		...
		private var keyReceived:Boolean = false;
		 ...
		 */
		public function FirebaseClient(useHandshake:Boolean = false, handshakeSignature:uint = 0):void
		{
			firebaseSocket = new FirebaseSocket(this, useHandshake, handshakeSignature);
		}
		...
		*/
		...
		private function onSendLater(event:SendLaterEvent):void
		{
			send(event.getObject());
		}
		...
		public function send(packet:ProtocolObject):void
		{
			...
			if ( _useEncryption ) 
			{
				if ( keyReceived == false ) {
					var sendLater:SendLater = new SendLater(packet);
					sendLater.addEventListener(SendLaterEvent.SEND_LATER, onSendLater);
					sendLater.schedule(500);
					return;
				}
