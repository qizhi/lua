...
public class DeleteTableProcessor extends ProcessorBase implements TableListener {

	private static final long serialVersionUID = -5695595327926401543L;

	@Override
	public void playerJoined(Table table, GenericPlayer player) {
		// table.getMetaData().getLobbyPath().setDomain("/1/1");
		// Logger.getLogger(getClass()).info("Setting domain incorrectly to: " + table.getMetaData().getLobbyPath());
	}
